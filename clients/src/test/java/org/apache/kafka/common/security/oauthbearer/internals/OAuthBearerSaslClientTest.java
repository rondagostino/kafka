...
public class OAuthBearerSaslClientTest extends EasyMockSupport {
    ...
    @Test
    public void testExposesExpiringCredentialAsNegotiatedProperty() throws SaslException {
        OAuthBearerSaslClient client = new OAuthBearerSaslClient(
                new TestCallbackHandler(TEST_TOKEN_ALSO_AS_EXPIRING_CREDENTIAL, SaslExtensions.NO_SASL_EXTENSIONS));
        client.evaluateChallenge(EMPTY_CHALLENGE);
        assertNull(client.evaluateChallenge(EMPTY_CHALLENGE));
        assertTrue(client.isComplete());
        assertSame(TEST_TOKEN_ALSO_AS_EXPIRING_CREDENTIAL,
                client.getNegotiatedProperty(ExpiringCredential.SASL_CLIENT_NEGOTIATED_PROPERTY_NAME));
    }

    @Test
    public void testDoesNotExposeNonExpiringCredentialAsNegotiatedProperty() throws SaslException {
        OAuthBearerSaslClient client = new OAuthBearerSaslClient(
                new TestCallbackHandler(TEST_TOKEN, SaslExtensions.NO_SASL_EXTENSIONS));
        client.evaluateChallenge(EMPTY_CHALLENGE);
        client.evaluateChallenge(EMPTY_CHALLENGE);
        assertNull(client.getNegotiatedProperty(ExpiringCredential.SASL_CLIENT_NEGOTIATED_PROPERTY_NAME));
    }
