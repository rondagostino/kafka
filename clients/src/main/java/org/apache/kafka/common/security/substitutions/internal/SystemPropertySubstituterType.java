...
/**
 * A {@code SubstituterType} that handles system property substitution. The
 * identifier typically indicates the system property to read. This substitution
 * type recognizes the following modifiers:
 * <ul>
 * <li>{@code redact} -- when enabled, the value of the system property is
 * stored such that it is prevented from being logged</li>
 * <li>{@code notBlank} -- when enabled, blank (only whitespace) or non-existent
 * system properties are replaced by default values. Implies
 * {@code notEmpty}.</li>
 * <li>{@code notEmpty} -- when enabled, either explicitly or via
 * {@code notBlank}, empty ({@code ""}) or non-existent system properties are
 * replaced by default values.</li>
 * <li>{@code fromValueOfKey} -- provides a level of indirection so that the
 * system property to be read, instead of being literally specified by the
 * identifier, can be determined via another key's value. The identifier is
 * interpreted as the key whose value identifies the system property to read.
 * This allows the system property name to potentially be generated from
 * multiple substitutions concatenated together.</li>
 * <li>{@code defaultValue=<value>} -- when enabled, the provided literal value
 * is used as a default value in case the system property either does not exist
 * or its value is disallowed via {@code notBlank} or {@code notEmpty}</li>
 * <li>{@code defaultKey=<key>} -- when enabled, the indicated key is evaluated
 * as a default value in case the system property either does not exist or its
 * value is disallowed via {@code notBlank} or {@code notEmpty}</li>
 * </ul>
 * 
 ...
 */
public class SystemPropertySubstituterType implements SubstituterType {
    private static final Logger log = LoggerFactory.getLogger(SystemPropertySubstituterType.class);
    private final SubstituterTypeHelper delegate;

    /**
     * Explicit default constructor
     */
    public SystemPropertySubstituterType() {
        this(null);
    }

    SystemPropertySubstituterType(final Map<String, RedactableObject> injectedValuesForTesting) {
        delegate = new SubstituterTypeHelper() {
            ...
            public RedactableObject retrieveResult(String type, String identifier, boolean redact,
                ...
                String retval = System.getProperty(identifier);
